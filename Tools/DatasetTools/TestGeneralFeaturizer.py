...
class TestGeneralFeaturizer(unittest.TestCase):
    ...
    def test_atoms_files_0(self):
        zerofiles = mod.AtomsObjects.file.map(lambda v: v[0])
        types = zerofiles.map(type)
        self.assertTrue(all(types == str))
    ...
    def test_df_has_cns(self):
        self.assertIn('sitecn', mod.df.columns)
